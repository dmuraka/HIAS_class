...
install.packages(c("sf","mapview","RColorBrewer","stringr","car")) # 初回のみ
library(sf)
library(mapview)
...
###### 地図を見てみる（dpriceから住宅地公示地価だけ抽出->dprice2）
# stringr: https://kazutan.github.io/kazutanR/stringr-intro.html
...
###### 余談：sf -> data.frame
#coords <-st_coordinates(st_centroid(price))
...
###### データの性質の確認
summary(dprice3)
dprice3 <- na.omit(dprice3)  # 欠損を含むデータの除去
price   <- dprice3$price
st_dist <- dprice3$st_dist
tk_dist <- dprice3$tk_dist

hist(price)
hist(log(price))

plot(log(price),log(st_dist))
plot(log(price),log(tk_dist))

###### 回帰モデルの推定
mod     <- lm(log(price)~log(st_dist)+log(tk_dist)+rice+agri+forest+wild+building+road+rail+other+river+ocean+golf,data=dprice3)
summary(mod)# 結果
vif(mod)    # 多重共線性の確認
...
land_dat1<-st_drop_geometry(dland[,c(2:14)]) # geometryを除去（sf->data.frame）
land_dat2<-rowSums(land_dat1[,-11])# 海(ocean)以外の土地利用の面積
